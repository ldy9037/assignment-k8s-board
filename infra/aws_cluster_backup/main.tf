...
data "aws_iam_policy_document" "eks_alb_controller_trust_policy" {
  statement {
    ...
    principals {
      type        = "Federated"
      identifiers = ["arn:aws:iam::${var.aws_account_id}:oidc-provider/oidc.eks.${var.region}.amazonaws.com/id/${var.eks_blog_cluster_oicd_id}"]
    }
    ...
    condition {
      test     = "StringEquals"
      variable = "oidc.eks.${var.region}.amazonaws.com/id/${var.eks_blog_cluster_oicd_id}:aud"
      values   = ["sts.amazonaws.com"]
    }

    condition {
      test     = "StringEquals"
      variable = "oidc.eks.${var.region}.amazonaws.com/id/${var.eks_blog_cluster_oicd_id}:sub"
...
module "eks_blog_role" {
  source  = "terraform-aws-modules/iam/aws//modules/iam-assumable-role"
  version = "5.9.1"

  create_role = var.eks_blog_role_create_role

  role_name = var.eks_blog_role_name
...
module "eks_blog_node_role" {
  source  = "terraform-aws-modules/iam/aws//modules/iam-assumable-role"
  version = "5.9.1"

  create_role = var.eks_blog_node_role_create_role

  role_name = var.eks_blog_node_role_name
...
resource "aws_eks_cluster" "eks_cluster_blog" {
  name     = var.eks_blog_cluster_name
  role_arn = module.eks_blog_role.iam_role_arn
  version  = var.eks_blog_cluster_version
...
resource "aws_eks_node_group" "eks_node_group_post" {
  cluster_name    = aws_eks_cluster.eks_cluster_blog.name
  node_group_name = var.node_group_post_name
  node_role_arn   = module.eks_blog_node_role.iam_role_arn
  subnet_ids      = data.tfe_outputs.network_output.values.private_subnets

  ami_type       = var.node_group_post_ami_type
  capacity_type  = var.node_group_post_capacity_type
  disk_size      = var.node_group_post_disk_size
  instance_types = var.node_group_post_instance_types

  scaling_config {
    desired_size = var.node_group_post_sc_desired_size
    max_size     = var.node_group_post_sc_max_size
    min_size     = var.node_group_post_sc_min_size
  }
